...
export default (state: ActivityState = initialState, action): ActivityState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_ACTIVITIES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_ACTIVITIES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_ACTIVITIES):
...
const apiSearchUrl = 'api/_search/activities';

// Actions

export const getSearchEntities: ICrudSearchAction<IActivity> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_ACTIVITIES,
  payload: axios.get<IActivity>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
